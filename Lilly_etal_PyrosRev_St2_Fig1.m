...
%% ======= Calculate no. observations per species =======
pspp_cnt = sum([height(lit_pspp_cln),height(cpd_pspp_cln),height(jdi_pspp_cln),...
                height(gbf_pspp_cln),height(obi_pspp_cln)]);
patl_cnt = sum([height(lit_patl_cln),height(jdi_patl_cln),...
                height(gbf_patl_cln),height(obi_patl_cln)]);
pahe_cnt = sum([height(lit_pahe_cln),height(obi_pahe_cln)]);
pova_cnt = sum([height(obi_pova_cln)]);
tspp_cnt = sum([height(gbf_tspp_cln)]);
tspi_cnt = sum([height(lit_tspi_cln),height(jdi_tspi_cln),...
                height(gbf_tspi_cln),height(obi_tspi_cln)]);
taga_cnt = sum([height(jdi_taga_cln),height(gbf_taga_cln),height(obi_taga_cln)]);
lspp_cnt = sum([height(gbf_lspp_cln),height(obi_lspp_cln)]);
lver_cnt = sum([height(lit_lver_cln),height(jdi_lver_cln),...
                height(gbf_lver_cln),height(obi_lver_cln)]);
